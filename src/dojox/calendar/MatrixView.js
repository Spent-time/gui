        ...
        return declare("dojox.calendar.MatrixView", [ViewBase, _TemplatedMixin], {
            ...
            _buildRowHeader: function (renderData, oldRenderData) {
                ...
                query("tr", rowHeaderTable).forEach(function (tr, i) {
                    ...
                    var d = renderData.dates[i] ? renderData.dates[i][0] : {};
                    ...
                    if (renderData.dates.length === 1) {

                        // find items with start and end date in this week
                        start = date.moment(renderData.dates[0][0]);
                        end = date.moment(renderData.dates[0][0]).add(7, 'days');

                    } else {

                        // find items with start and end date in this week
                        start = date.moment(renderData.dates[i][0]);
                        end = date.moment(renderData.dates[i][0]).add(7, 'days');
                    }
                    ...
                    for (var e = 0; e < items.length; e++) {
                        ...
                        // match if the item belongs to today
                        var item = items[e];
                        var checkDate = date.moment(item.startTime);

                        if (checkDate.isBetween(start, end)) {

                            totalTime += date.duration(item);
                        }
                    }
                    ...
                    if (totalTime > 0) {
                        ...
                        var totalHours = Math.trunc(totalTime);
                        ...
                        label = '<div class="totalWeek">' + label + '</div>';
                    }

                    var r = this._formatRowHeaderLabel(d);
            ...
            _buildGrid: function (renderData, oldRenderData) {
                ...
                query("tr", table).forEach(function (tr, row) {
                    ...
                    query("td", tr).forEach(function (td, col) {
                        ...
                        if (this.getParent().viewKind === 'columns') {

                            // time counter for the whole day
                            var totalTime = 0;

                            // format to compare
                            var day = date.moment(d).format('YYYY-MM-DD');

                            // these are the selected items for the date and time period selected
                            // week or day
                            var items = renderData.items;
                            ...
                            for (var e = 0; e < items.length; e++) {
                                ...
                                // match if the item belongs to today
                                var item = items[e];
                                var start = date.moment(item.startTime).format('YYYY-MM-DD');
                            ...
                            if (totalTime > 0) {
                                ...
                                label = '<div class="totalDay">' + label + '</div>';
                            }

                            this._setText(span, label, true);
