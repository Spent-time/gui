        ...
        return declare("dojox.calendar.SimpleColumnView", [ViewBase, _TemplatedMixin], {
            ...
            _buildColumnHeader: function (renderData, oldRenderData) {
                ...
                if (this.yearColumnHeaderContent) {
                    ...
                    var y = renderData.dateLocaleModule.format(d,
                        {selector: "date", datePattern: "yyyy"});

                    var w = renderData.dateLocaleModule.format(d,
                        {selector: "date", datePattern: "ww"});

                    var label = w + ' - ' + y;

                    this._setText(this.yearColumnHeaderContent, label);
                }
            },
            ...
            _layoutTimeIndicator: function (renderData) {
                ...
                if (this.showTimeIndicator) {
                    ...
                    if (visible) {
                        ...
                        var tracked = runtime.calendar.store.query({
                            tracked: true
                        });
                        ...
                        if (!this._timeIndicator) {
                            ...
                            var track = domConstruct.create("div", {
                                "innerHTML": "Start",
                                "className": "timeTrackButton",
                                "id": "timeTrackButton"
                            });

                            this._timeIndicator.appendChild(track);

                            on(track, 'click', dojo.hitch(this, function (e) {
                                ...
                                if (tracked.length === 1) {
                                    ...
                                    // already tracking, so we are stopping
                                    tracked[0].tracked = false;

                                    runtime.calendar.store.put(tracked[0]);

                                    e.target.innerHTML = 'Start';

                                } else {

                                    // not tracking, so we start tracking
                                    e.trackingStart = true;

                                    this._onGridDoubleClick(e);
                        ...
                        if (tracked.length === 1) {
                            ...
                            if (runtime.calendar.columnView.minDurationStepsUnit === 'minute') {

                                multiplier = 60;

                            } else if (runtime.calendar.columnView.minDurationStepsUnit === 'hour') {

                                multiplier = 3600;

                            } else if (runtime.calendar.columnView.minDurationStepsUnit === 'day') {

                                multiplier = 86400;
                            }

                            var minDurationStepDiff = runtime.calendar.columnView.minDurationSteps * multiplier * 1000;

                            if (date.diff(tracked[0]) > minDurationStepDiff) {

                                tracked[0].endTime = date.now();

                                runtime.calendar.store.put(tracked[0]);
                            }

                            node.firstChild.innerHTML = 'Stop';
                        }
