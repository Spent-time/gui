define([
    ...
    function (declare,
        ...
        return declare("dojox.calendar.CalendarBase", [_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, StoreMixin, _Invalidating, Selection], {
            ...
            // indicates if the today view has been selected
            today: false,
            ...
            nextRange: function () {
                ...
                // check if we are in today view
                // if so we switch to day view when range changes
                if (this.today === true) {

                    this.today = false;
                    // so we are moving away from today
                    // which makes it another day than today
                    domClass.add(this.dayButton.domNode, 'selected');
                    domClass.remove(this.todayButton.domNode, 'selected');
                    domClass.remove(this.weekButton.domNode, 'selected');
                    domClass.remove(this.monthButton.domNode, 'selected');
                }
            ...
            postCreate: function () {
                ...
                switch (this.dateInterval) {
                    case 'day':
                        ...
                        break;
                    case 'week':
                        domClass.add(this.weekButton.domNode, 'selected');
                        break;
                    case 'month':
                        domClass.add(this.monthButton.domNode, 'selected');
                        break;
                }
            },
            ...
            todayButtonClick: function (e) {
                ...
                // set today selected flag
                this.today = true;

                // toggle selected class
                domClass.add(this.todayButton.domNode, 'selected');
                domClass.remove(this.dayButton.domNode, 'selected');
                domClass.remove(this.weekButton.domNode, 'selected');
                domClass.remove(this.monthButton.domNode, 'selected');
            ...
            dayButtonClick: function (e) {
                ...
                // unset possible true today flag
                this.today = false;
