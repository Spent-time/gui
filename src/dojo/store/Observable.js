define([
    ...
    "../_base/array",
    'apps/shared/runtime'
], function (client, kernel, lang, when, array, runtime /*=====, Store =====*/) {
...
//		dojo/store/Observable
    ...
    var Observable = function (/*Store*/ store) {
        ...
        client.subscribe(
            client[store.collection].created, function (arr) {
                ...
                if (store.collection === 'item'
                    && runtime.tree) {

                    var path = runtime.tree.model.store.getParentPath(lastItem.id);
                    runtime.tree.set('path', path);

                    var node = runtime.tree._itemNodesMap[lastItem.id];
                    node[0].domNode.scrollIntoView();
                }
            });
